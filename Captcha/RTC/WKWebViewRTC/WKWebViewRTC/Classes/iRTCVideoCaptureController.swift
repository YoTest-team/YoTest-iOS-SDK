...
class iRTCVideoCaptureController : NSObject {
	...
	*/
    ...
    @discardableResult func startCapture() -> Bool {
		...
		if (device == nil) {
			NSLog("iRTCVideoCaptureController#startCapture No matching device found for constraints %@", constraints);
			return false;
		}
		
		if (deviceFormat == nil) {
			NSLog("iRTCVideoCaptureController#startCapture No valid formats for device %@", device!);
			return false;
		}
		...
		NSLog("iRTCVideoCaptureController#startCapture Capture started, device:%@, format:%@", device!, deviceFormat!);
	...
	func stopCapture() {
		...
		if (isCapturing) {
			...
			NSLog("iRTCVideoCaptureController#stopCapture Capture stopped");
		}
	}
	...
	fileprivate func findDevice() -> AVCaptureDevice? {
		...
		if (facingMode.count > 0) {
			...
			if (facingModeRequired && position == AVCaptureDevice.Position.unspecified) {
				NSLog("iRTCVideoCaptureController#findDevice facingMode fail exact requirement");
				return nil;
			}
			
			NSLog("iRTCVideoCaptureController#findDevice facingMode:%@ env:%@ user:%@", facingMode,
		...
		if (deviceId.count > 0) {
			...
			if (device == nil && self.isConstrainDOMStringExact(constraint: "deviceId")) {
				NSLog("iRTCVideoCaptureController#findDevice deviceId fail exact requirement");
				return nil;
			}
		}
		...
		NSLog("iRTCVideoCaptureController#findDevice device:%@", device!);
		...
		if (minFrameRate > 0 || maxFrameRate > 0) {
			...
			if (
				...
				NSLog("iRTCVideoCaptureController#findDevice frameRate fail exact requirement");
				return nil;
			}
			
			deviceFrameRate = Int(maxFrameRate > 0 ? maxFrameRate : minFrameRate)
			
			NSLog("iRTCVideoCaptureController#findDevice deviceFrameRate:%i", deviceFrameRate!);
	...
	*/
	...
	fileprivate func findFormatForDevice(device: AVCaptureDevice) -> AVCaptureDevice.Format? {
		...
		if (minAspectRatio > 0 || maxAspectRatio  > 0) {
			...
			NSLog("iRTCVideoCaptureController#findFormatForDevice contraints using aspectRatioRange - width:%i/%i, height:%i/%i, aspectRatio: %f/%f", minWidth, maxWidth, minHeight, maxHeight, minAspectRatio, maxAspectRatio);
		...
		// If no aspectRatioRange and no dimension is provided use default dimension
			...
			NSLog("iRTCVideoCaptureController#findFormatForDevice contraints using default - width:%i/%i, height:%i/%i, aspectRatio: %f/%f", minWidth, maxWidth, minHeight, maxHeight, minAspectRatio, maxAspectRatio);
		}
		...
		for format: Any in formats {
			...
			NSLog("iRTCVideoCaptureController#findFormatForDevice device format - width:%i, height:%i, aspectRatio: %f, frameRateRanges:%f/%f", dimension.width, dimension.height, aspectRatio, frameRates.minFrameRate, frameRates.maxFrameRate);
		...
		if (selectedFormat != nil) {
			...
			NSLog("iRTCVideoCaptureController#findFormatForDevice selected format - width:%i, height:%i, aspectRatio: %f, frameRateRanges:%f/%f", dimension.width, dimension.height, aspectRatio, frameRates.minFrameRate, frameRates.maxFrameRate);
		}
